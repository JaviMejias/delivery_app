...
import {
  ...
  RefreshCw, User, Phone, FileText, Zap
} from 'lucide-react'
    ...
    if (radarData.truck.latitude && radarData.truck.longitude) {
      ...
      const truckIconHtml = `
        <div class="relative flex flex-col items-center">
          <div class="flex items-center justify-center w-12 h-12 rounded-full bg-indigo-600 border-3 border-white shadow-2xl shadow-indigo-500/50">
          ...
          <div class="mt-1 px-2 py-0.5 bg-slate-900 text-indigo-300 text-[9px] font-black rounded border border-indigo-500/30">Tú</div>
        </div>
      `
    ...
    <AuthenticatedLayout hideHeader={isDriver}>
      ...
      <div className={`flex flex-col overflow-hidden relative ${isDriver ? 'h-[100dvh] w-full' : 'h-[calc(100vh-5rem)] lg:h-[calc(100vh-9.5rem)] -mx-4 -my-4 lg:-mx-8 lg:-my-8 bg-[var(--sf-bg)] rounded-3xl'}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 
                  className="text-xl font-black text-white tracking-tight active:scale-95 transition-transform"
        ...
        <div className="flex-1 flex overflow-hidden relative">
          ...
          <div id="radar-map" className="flex-1 h-full z-0 bg-slate-950" />
          ...
          {!isFollowing && (
            <button 
              ...
              className="absolute top-20 right-4 md:bottom-6 md:top-auto md:right-6 z-[400] w-12 h-12 md:w-14 md:h-14 rounded-full bg-slate-800 border-2 border-slate-700 shadow-2xl flex items-center justify-center text-slate-300 hover:text-white hover:bg-slate-700 transition-colors pointer-events-auto"
          ...
          <div className="absolute inset-0 z-10 pointer-events-none p-3 md:p-4">
            <div className="flex flex-col w-full md:w-[22rem] gap-2">
              ...
              <div className="flex flex-col gap-2 mt-0">
                ...
                {data?.truck.has_destination && (
                  <div className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-indigo-600/90 text-white font-bold text-xs pointer-events-auto shadow-xl">
          ...
          >
            <div className={`flex flex-col shrink-0 max-h-[75vh] md:max-h-[70vh] pointer-events-auto backdrop-blur-2xl rounded-t-[2rem] border-t border-white/10 md:rounded-2xl md:border shadow-[0_-10px_40px_rgba(0,0,0,0.5)] overflow-hidden ${
              data?.truck.has_destination ? 'bg-gradient-to-br from-indigo-900/95 to-slate-900/95' : 'bg-slate-950/95'
            }`}>
              
              <div 
                className="w-full h-14 shrink-0 flex flex-col justify-center items-center cursor-pointer hover:bg-white/5 transition-colors md:hidden"
              ...
              <div className="overflow-y-auto scrollbar-hide pb-8 md:pb-0 flex flex-col min-h-0">
                {data && data.orders.some((o: any) => o.is_proposed) && data.truck.active_order_id && (
                  <div 
                    className="p-3 mb-2 mx-3 bg-indigo-600/90 backdrop-blur-md border border-indigo-400 rounded-xl shadow-[0_0_20px_rgba(79,70,229,0.3)] flex items-center justify-between cursor-pointer hover:bg-indigo-500 transition-colors shrink-0"
                    onClick={() => {
                      const proposed = data.orders.find((o: any) => o.is_proposed)
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="p-2 bg-indigo-900/50 rounded-full animate-pulse">
                        <AlertCircle className="w-5 h-5 text-white" />
                      </div>
                      <div>
                        <p className="text-white font-black text-xs uppercase tracking-wider">Despacho Propuesto</p>
                        <p className="text-indigo-100 text-[10px]">Administración te ha asignado un nuevo pedido</p>
                      </div>
                    </div>
