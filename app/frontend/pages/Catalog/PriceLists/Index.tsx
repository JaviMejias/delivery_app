...
export default function PriceListsIndex({ priceLists, pagination }: Props) {
  ...
  const deleteList = (id: number) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        router.delete(`/catalog/price-lists/${id}`, {
          onSuccess: () => {
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-1" ref={formRef}>
            <Card className={editingList ? 'ring-2 ring-indigo-500 shadow-[0_0_30px_rgba(99,102,241,0.2)] transition-all duration-300' : 'transition-all duration-300'}>
              <Card.Body>
                ...
                <Table>
                  ...
                  <Table.Tbody>
                    {priceLists.length === 0 ? (
                      ...
                      priceLists.map((l) => (
                        <Table.Tr key={l.id} className={editingList?.id === l.id ? 'bg-indigo-500/5' : ''}>
                          ...
                          <Table.Td>
                            ...
                            <div className="flex gap-1.5 mt-2">
                              {l.available_for_trucks && <span className="px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wider font-bold bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">Camiones</span>}
                          ...
                          <Table.Td className="text-right">
                            <div className="flex items-center justify-end gap-3">
                              <button onClick={() => editList(l)} className="text-indigo-400 hover:text-indigo-300 font-medium flex items-center gap-1.5 transition-colors">
                                <Pencil className="w-3.5 h-3.5" /> Editar
                              </button>
                              <button onClick={() => deleteList(l.id)} className="text-red-400 hover:text-red-300 font-medium flex items-center gap-1.5 transition-colors">
