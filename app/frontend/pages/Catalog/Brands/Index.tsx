  ...
  const deleteBrand = (id: number) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        router.delete(`/catalog/brands/${id}`, {
          onSuccess: () => {
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1" ref={formRef}>
            <Card className={editingBrand ? 'ring-2 ring-indigo-500 shadow-[0_0_30px_rgba(99,102,241,0.2)] transition-all duration-300' : 'transition-all duration-300'}>
              <Card.Body>
          ...
          <div className="lg:col-span-2">
            <Card className="overflow-hidden flex flex-col h-full">
              <div className="p-4 border-b border-[var(--sf-border)] bg-[var(--sf-surface)]">
                <SearchBar routeName="/catalog/brands" currentSearch={currentSearch || ""} placeholder="Buscar por nombre..." />
              </div>
                ...
                <Table>
                  ...
                  <Table.Tbody>
                    {brands.length === 0 ? (
                      ...
                      brands.map((b) => (
                        <Table.Tr key={b.id} className={editingBrand?.id === b.id ? 'bg-indigo-500/5' : ''}>
                          ...
                          <Table.Td className="text-right">
                            <div className="flex items-center justify-end gap-3">
                              <button onClick={() => editBrand(b)} className="text-indigo-400 hover:text-indigo-300 font-medium flex items-center gap-1.5 transition-colors">
                                <Pencil className="w-3.5 h-3.5" /> Editar
                              </button>
                              <button onClick={() => deleteBrand(b.id)} className="text-red-400 hover:text-red-300 font-medium flex items-center gap-1.5 transition-colors">
