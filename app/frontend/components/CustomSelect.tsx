...
const customClassNames = {
  control: (state: any) =>
    `flex items-center justify-between w-full !bg-[var(--sf-surface)] !border-[var(--sf-border)] !border !rounded-xl !min-h-[46px] shadow-sm hover:!border-indigo-500/50 transition-all duration-200 cursor-text ${
      state.isFocused ? '!ring-4 !ring-indigo-500/20 !border-indigo-500' : ''
    }`,
  ...
  dropdownIndicator: (state: any) =>
    `p-2 hover:!text-indigo-400 transition-colors cursor-pointer ${state.isFocused ? '!text-indigo-400' : ''}`,
  clearIndicator: () => 'p-2 hover:!text-red-400 transition-colors cursor-pointer',
  menu: () => 'absolute top-[100%] w-full !bg-[var(--sf-surface)] !border !border-[var(--sf-border)] !rounded-2xl !shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:!shadow-[0_8px_30px_rgb(0,0,0,0.5)] !mt-2 !overflow-hidden !z-50 backdrop-blur-xl',
  ...
  option: (state: any) =>
    `px-3 py-2.5 rounded-xl cursor-pointer transition-all duration-200 flex items-center mb-1 last:mb-0 ${
      state.isSelected
        ? 'bg-gradient-to-r from-indigo-500 to-indigo-600 text-white font-medium shadow-md shadow-indigo-500/20'
        : state.isFocused
